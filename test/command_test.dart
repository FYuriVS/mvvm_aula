...
void main() {
  group(
    ...
    () {
      ...
      test(
        "Should test Command0 returns Error",
        () async {
      ...
      test(
        "Should teste Command1 ok Result",
        () async {
          final command1 = Command1<String, String>(getOkResult1);

          expect(command1.runnig, false);
      ...
      test(
        "Should teste Command1 Error Result",
        () async {
...
Future<Result<String>> getErrorResult() async {
  await Future.delayed(const Duration(milliseconds: 500));
  return Result.error(Exception("Error in operation"));
}
